...
class FLClientBase(ABC):
    ...
    # ══════════════════════════════════════════════════════════════════════
    # 探针状态协议（Exp 0.3 paired counterfactual）
    # ══════════════════════════════════════════════════════════════════════
    #
    # `self.model` 之外的**持久个性化状态**——FedRep 的私有 head、pFedMe 的锚点 Θ、
    # Ditto 的 v_k。checkpoint 必须连它一起存，否则从 θ_t 复现出来的客户端跑的是
    # 「个性化状态回到初始化」的另一条轨迹，与真实第 t 轮无关。
    #
    # 为什么由方法类自己声明而不是探针去猜：这些状态的语义各不相同，探针无从判断
    # 哪个属性是"个性化状态"、哪个只是缓存。猜错的后果是静默丢状态（陷阱 #7 那一类）。

    def get_probe_state(self) -> dict:
        """
        本方法的持久个性化状态，{名字: [np.ndarray, ...]}。默认空 —— 无个性化状态的
        方法（Hier-FedAvg）本来就只有 self.model，不需要额外声明。
        """
        return {}

    def set_probe_state(self, state: dict):
        """
        装载 get_probe_state 的产物。

        **未知键必须报错**，不能忽略：忽略的表现是「checkpoint 里有、装载后没有」，
        而这在数值上等同于"该客户端的个性化状态被清零"，没有任何日志会提到它。
        """
        if state:
            raise ValueError(
                f"{type(self).__name__} 没有实现 set_probe_state，但 checkpoint 里带了 "
                f"{sorted(state)}。要在这个方法上跑探针，先实现 get/set_probe_state。")
