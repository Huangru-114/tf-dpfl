"""
server/hier_fedrep.py  –  Hier-FedRep 边缘服务器

三层结构（本质 = Hier-FedAvg + 私有 head）：
  Cloud:  全局 backbone φ*    标准 FedAvg 聚合各 edge 上传的 backbone（server.py 不改）
  Edge:   edge backbone φ_e   仅对 backbone 索引做样本加权 FedAvg
  Client: 共享 backbone + 私有 head（client/hier_fedrep.py）

与 Hier-Ditto-Rep 的 edge 层相比，去掉了 Ditto 近端正则（不把 φ_e 拉向 φ*），
就是最朴素的 backbone-only FedAvg。
